...
def main(db_name, db_credential):
    ...
    # df = pd.read_csv('data/B1/cards_data.csv')
    # engine = create_engine(db_credential + db_name)
    # table_name = 'b1_cards_data'
    # df.to_sql(table_name, engine, if_exists='replace', index=False)

    # print("  mcc_codes loading...")
    # with open('data/B1/mcc_codes.json') as f:
    #     data = json.load(f)
    ...
    # df = pd.read_csv('data/B1/transactions_data.csv')
    # engine = create_engine(db_credential + db_name)
    # table_name = 'b1_transactions_data'
    # df.to_sql(table_name, engine, if_exists='replace', index=False)

    # print("  users_data loading...")
    # df = pd.read_csv('data/B1/users_data.csv')
    # engine = create_engine(db_credential + db_name)
    ...
    df = pd.read_csv('data/B3/Online_Sales.csv')
    table_name = 'b3_online_sales'
    df.to_sql(table_name, engine, if_exists='replace', index=False)

    df = pd.read_excel('data/B3/CustomersData.xlsx', header=0)
